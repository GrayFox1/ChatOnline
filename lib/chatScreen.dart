...
Future<Null> verificaLoggedIn() async{
  ...
  if(await auth.currentUser() == null){
    ...
    AuthCredential credential = GoogleAuthProvider.getCredential(idToken: credentials.idToken, accessToken: credentials.accessToken);
    await auth.signInWithCredential(credential);

    await auth.linkWithCredential(GoogleAuthProvider.getCredential(
...
handleSubmitted(String text) async{
  await verificaLoggedIn().then((temp) {
          ...
          children: <Widget>[
            Container(
              child: IconButton(
                ...
                onPressed: () async{
                  await verificaLoggedIn().then( (temp) async {
            ...
            Container(
              ...
              child: Theme.of(context).platform == TargetPlatform.iOS ? CupertinoButton(
                child: Text("Enviar"),
                onPressed: isEditing ? () {
                  handleSubmitted(msgController.text);
                  reset();
                } : null,
                ) : 
                IconButton(
...
class ChatMessage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 14.0, horizontal: 15.0),
      child: Row(
              ...
              children: <Widget>[
                ...
                Container(
                  ...
                  child: data["imgUrl"] != null ? 
                    Image.network(data["imgUrl"], width: 150.0, height: 150.0) 
                    : Text(data["text"]) ,
                ),
              ],
            ),
